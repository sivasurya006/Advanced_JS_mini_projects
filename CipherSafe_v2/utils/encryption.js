// sBox 
const sBox = [
    
];

// inverse sBox
const invSbox = new Array(256);                          
for(let i=0;i<256;i++){
    invSbox[sBox[i]] = i;
}

// console.log(sBox[0]);
// console.log(invSbox[99]);

const rCon = [0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36];           // round Constants useful for key expansion  (for AES-128 rounds 1..10)


 // helper function to find 2.a
function xtime(a){
    a = a & 0xff;                                                           //it remove the bits over 8.  byte masking   1111 1111. ensure only 8 bits.  anything above 8 bits is cut off.
    return (a & 0x80) ? (((a << 1) & 0xff) ^ 0x1B) : ((a << 1) & 0xff);     // ( a & 0x80 ) it check the fist bit is 1 or 0. detailed explanation in my note. 
}

// MixColumns helpers
function mixOneColumn(column){
    let copyCol = Array.from(column);
    let b0 = xtime(copyCol[0]);
    let b1 = xtime(copyCol[1]);
    let b2 = xtime(copyCol[2]);
    let b3 = xtime(copyCol[3]);
  
    let newc0Val = b0 ^ (b1 ^ copyCol[1]) ^ copyCol[2] ^ copyCol[3];    // 2 3 1 1
    let newc1Val = copyCol[0] ^ b1 ^ (b2 ^ copyCol[2]) ^ copyCol[3];   // 1 2 3 1
    let newc2Val = copyCol[0] ^ copyCol[1] ^ b2 ^ (b3 ^ copyCol[3]);   // 1 1 2 3
    let newc3Val = (b0 ^ copyCol[0]) ^ copyCol[1] ^ copyCol[2] ^ b3;    // 3 1 1 2
  
    column[0] = newc0Val & 0xff;
    column[1] = newc1Val & 0xff;
    column[2] = newc2Val & 0xff;
    column[3] = newc3Val & 0xff;
}



function addRoundKey(currentState, key){                                     // need to clarify
    for(let row=0; row<4; row++){
      for(let col=0; col<4; col++){
        const keyIndex = 4*col + row;
        currentState[row][col] = (currentState[row][col] ^ key[keyIndex]) & 0xff;
      }
    }
}

// SubBytes / InvSubBytes
function sub_bytes(state){
    for(let i=0;i<4;i++){
        for(let j=0;j<4;j++){
            state[i][j] = sBox[state[i][j]];
        }
    }
}


// ShiftRows / InvShiftRows
function shiftRows(state){
    for(let i=0;i<4;i++){
        shift(state[i], i);
    }
}
function shift(arr,count){
    for(let k=0;k<count;k++){
      let temp = arr[0];
      for(let j=0;j<3;j++){
        arr[j] = arr[j+1];
      } 
      arr[3] = temp;
    }
}

// MixColumns 
function mixColumns(state){
    for(let col=0; col<4; col++){
      let column = [];
      for(let row=0; row<4; row++){
        column[row] = state[row][col]
      }
      mixOneColumn(column);
      for(let row=0; row<4; row++){
        state[row][col] = column[row]
      }
    }
}


// Key schedule helpers
function rotateWord(word){
    return [word[1], word[2], word[3], word[0]];
}
function sub_word(word){
    for(let i=0;i<4;i++){
        word[i] = sBox[word[i]];
    } 
    return word; 
}
function xorWords(a,b){
    return [a[0]^b[0], a[1]^b[1], a[2]^b[2], a[3]^b[3]];
}


// KeyScheduler (uses Rcon table)
function keyScheduler(roundKey){
    // roundKey: array of 16 bytes
    let words = new Array(44);

    // fill first 4 words
    for(let i=0;i<4;i++){
      words[i] = [];
      for(let j=0;j<4;j++){
        words[i][j] = roundKey[i*4 + j] & 0xff;
      }
    }

    for(let i=4;i<44;i++){
      let temp = Array.from(words[i-1]);
      if(i % 4 === 0){
        temp = sub_word(rotateWord(temp));
        temp[0] = temp[0] ^ rCon[(i/4)-1]; // XOR rcon into first byte
      }
      words[i] = xorWords(words[i-4], temp);
    }
  
    return words.flat(); // 176 bytes (44 words * 4)
  }

// AES encrypt single 16-byte block 
function AES_encrypt_block(state, expandedKey){
    // initial round
    addRoundKey(state, expandedKey.slice(0,16));
    // 9 rounds
    for(let round=1; round<=9; round++){
      sub_bytes(state);
      shiftRows(state);
      mixColumns(state);
      addRoundKey(state, expandedKey.slice(round*16, (round+1)*16));
    }
    // final round
    sub_bytes(state);
    shiftRows(state);
    addRoundKey(state, expandedKey.slice(10*16, 11*16)); // last key
}


export function AES_encrypt(message){
    let messageStates = [];

    for (let k = 0; k < Math.ceil(message.length / 16); k++) {
        let messageBytes = [];
        for (let i = 0; i < 16; i++) {
            let idx = k * 16 + i;                 
            messageBytes.push(idx < message.length ? message.charCodeAt(idx) : 0);
        }

        let state = [[], [], [], []];
        for (let c = 0; c < 4; c++) {
            for (let r = 0; r < 4; r++) {
                state[r][c] = messageBytes[c * 4 + r];
            }
        }

        messageStates.push(state);
    }

    // key
    let roundKey = [];
    for(let i=0;i<16;i++){                    // key length must 128 bits because I implement AES-128, i try to generate the main key randomly.
        roundKey[i] = Math.floor(Math.random()*256);
    }

    let expanded=keyScheduler(roundKey);

    let cipherHexAll = "";

    for (let i = 0; i < messageStates.length; i++) {
    
        // encrypt
        AES_encrypt_block(messageStates[i],expanded);
        let cipherState = messageStates[i].flat();
        cipherHexAll += cipherState.map(v => v.toString(16)).join("");
    }

    const chipherText = messageStates.flat(2).map(v => v.toString(16)).join(',')
    const key = roundKey.map(v => v.toString(16)).join(',')

    return [chipherText,key];
}







